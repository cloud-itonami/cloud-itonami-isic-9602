(ns salon.operation
  "OperationActor -- one hairdressing/beauty-treatment operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (SalonOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Personal Service Safety
  ...
  One graph run = one hairdressing/beauty-treatment operation (intake
  ...
  decision to a human operator (the licensed practitioner). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:treatment/perform` ALWAYS reaches this node when the
  governor is clean -- see `salon.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [salon.salonopsllm :as salonopsllm]
            [salon.governor :as governor]
...
(defn build
  ...
  `salon.store/Store`).
  opts:
    :advisor      -- a `salon.salonopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (salonopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; SalonOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (salonopsllm/-advise advisor store request)]
            {:proposal p :audit [(salonopsllm/trace request p)]})))

      ;; Personal Service Safety Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
